...
describe("Client", function() {
    ...
    describe("constructor", function() {
        it("should throw a TypeError if strategies are missing", function() {
            assert.throws(() => {
                new Client({});
            }, TypeError, "strategies must be an array.");
        });
    ...
    describe("post", function() {
        it("should call post on each strategy", async function() {
            const strategies = [
                { name: "test1", post() { return Promise.resolve("test1"); } },
                { name: "test2", post() { return Promise.resolve("test2"); } }
            ];
        ...
        it("should throw an error when one strategy fails", async function() {
            const strategies = [
                { name: "test1", post() { return Promise.resolve("test1"); } },
                { name: "test2", post() { return Promise.reject(new Error("test2")); } }
            ];
        ...
        it("should throw an error when multiple strategies fail", async function() {
            const strategies = [
                { name: "test1", post() { return Promise.reject(new Error("test1")); } },
                { name: "test2", post() { return Promise.reject(new Error("test2")); } }
            ];
